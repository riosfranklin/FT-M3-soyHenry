...
function problemA() {
  ...
  // promise version
  // ???
  var p1 = promisifiedReadFile("poem-two/stanza-01.txt").then((stanza1) =>
    blue(stanza1)
  );
  var p2 = promisifiedReadFile("poem-two/stanza-02.txt").then((stanza2) =>
    blue(stanza2)
  );
...
function problemB() {
  ...
  // promise version
  // ???
  let allPromises = filenames.map(function (n) {
    return promisifiedReadFile(n);
  });

  Promise.all(allPromises).then((stanzas) => {
...
function problemC() {
  ...
  // promise version
  // ???
  // let allPromises = filenames.map((n) => {
  //   return promisifiedReadFile(n);
  // });
  // Promise.all(allPromises).then((stanzas) => {
